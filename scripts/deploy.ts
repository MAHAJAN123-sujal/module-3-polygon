...
const BASE_PATH = "./circuits/MahajanCustomCircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/MahajanCustomCircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/MahajanCustomCircuitVerifier.sol:Verifier");
